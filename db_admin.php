...
if (isset($_POST["action"])) {
  ...
  if ($_POST["action"] == "insert") {
    ...
    $sql = "insert into table_admin values('$id_admin','$nama',
    '$username','$password')";
    ...
    //     unlink("img_pustakawan/".$hasil["image"]);
      ...
      // simpan file gambar
      $sql = "update table_admin set nama='$nama',
...
if (isset($_GET["hapus"])) {
  ...
  $sql = "select * from table_admin where id_admin='$id_admin'";
  // eksekusi query
  $result = mysqli_query($koneksi,$sql);
  // konversi ke array
  $hasil = mysqli_fetch_array($result);

  $sql = "delete from table_admin where id_admin='$id_admin'";
  if (mysqli_query($koneksi,$sql)) {
    // jika query sukses
    $_SESSION["message"] = array(
      "type" => "success",
